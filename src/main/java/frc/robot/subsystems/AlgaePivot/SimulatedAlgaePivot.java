...
public class SimulatedAlgaePivot implements AlgaePivotInterface{
  ...
  private SingleJointedArmSim algaePivotSim = new SingleJointedArmSim(
  ...
  public void setAlgaeAngle(double angle){
    ...
    double armFF = armFeedForward.calculate(
      angle, algaePivotController.getSetpoint().velocity);
    setVoltage(algaePivotController.calculate(angle, currentAlgaePivotAngleRots) + armFF);
  }
}

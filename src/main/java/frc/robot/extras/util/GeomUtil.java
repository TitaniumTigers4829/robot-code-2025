 ...
 */
public static boolean areTranslationsWithinThreshold(double thresholdMeters, Translation2d... translations) {
...
*/
public static boolean areRotationsWithinThreshold(double thresholdDegrees, Rotation2d... rotations) {
...
/**
 * Checks if all pairs of poses in the input list are within the given translation and rotation thresholds.
 *
 * @param translationThresholdMeters the maximum allowed distance between any two translations in meters
 * @param rotationThresholdDegrees   the maximum allowed difference between any two rotations in degrees
 * @param poses                      a list of Pose2d objects
 * @return true if all pairs of poses are within the given thresholds, false otherwise
 */
public static boolean arePosesWithinThreshold(double translationThresholdMeters, double rotationThresholdDegrees, Pose2d... poses) {
    int n = poses.length;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            ...
            if (poses[i].getTranslation().getDistance(poses[j].getTranslation()) > translationThresholdMeters) {
                return false; // Return false if any pair of translations exceeds the threshold
            }

            // Check rotation threshold
            if (Math.abs(poses[i].getRotation().minus(poses[j].getRotation()).getDegrees()) > rotationThresholdDegrees) {

...
public class CrescendoFieldSimulation extends SimulatedField {
  ...
  public void placeGamePiecesOnField() {
    for (Translation2d notePosition : NOTE_INITIAL_POSITIONS)
      super.addGamePiece(new CrescendoNoteSimulation(notePosition));
  }
  ...
  public void competitionPeriodic() {
    ...
    final Translation2d sourcePosition;

    if (AllianceFlipper.isBlue()) {
      sourcePosition = BLUE_SOURCE_POSITION;
    } else {
      sourcePosition = AllianceFlipper.flipTranslation(BLUE_SOURCE_POSITION);
    }

    /* if there is any game-piece 0.5 meters within the human player station, we don't throw a new note */
    for (GamePieceSimulation gamePiece : super.gamePieces)
      if (gamePiece instanceof CrescendoNoteSimulation
          && gamePiece.getPoseOnField().getTranslation().getDistance(sourcePosition) < 1) return;

    /* otherwise, place a note */
    addGamePiece(new CrescendoNoteSimulation(sourcePosition));

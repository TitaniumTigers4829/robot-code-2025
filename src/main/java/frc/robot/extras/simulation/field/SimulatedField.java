...
public abstract class SimulatedField {
  private static SimulatedField instance = null;
   ...
   *
   * <p>Multiple instances of {@link SimulatedField} can exist elsewhere.
   *
   * @return the main simulation arena instance
   */
  public static SimulatedField getInstance() {
    if (instance == null) instance = new CrescendoFieldSimulation();
    return instance;
  }
   ...
   */
  public static void overrideInstance(SimulatedField newInstance) {
  ...
  public static double SIMULATION_DT = 0.02 / SIMULATION_SUB_TICKS_IN_1_PERIOD;
   ...
   *
   * <p>Changes apply to every instance of {@link SimulatedField}.
   *
   * <p>The new configuration will take effect the next time {@link
   * SimulatedField#simulationPeriodic()} is called on an instance.
   *
   * <p>It is recommended to call this method before the first call to {@link
   * SimulatedField#simulationPeriodic()} of any instance.
   *
   ...
   *     simulation
   */
  protected SimulatedField(FieldMap obstaclesMap) {
   ...
   * to {@link SimulatedField#simulationPeriodic()}.
   *
   * @param gamePiece the game piece to be registered in the simulation
   */
  public void addGamePiece(CrescendoNoteSimulation gamePiece) {
  ...
  public void clearGamePieces() {
    for (GamePieceSimulation gamePiece : this.gamePieces) this.physicsWorld.removeBody(gamePiece);
   ...
   *
   * <p>If not configured through {@link SimulatedField#overrideSimulationTimings(double
   ...
   *       SimulatedField#addSimulationSubTickAction(Runnable)}.
   * </ul>
   */
   ...
   * SimulatedField} to reflect the unique game piece placements for that season's game.
   */
   ...
   *
   *
   * <h2>Season-Specific Actions to Execute in {@link SimulatedField#simulationPeriodic()}.</h2>
   *
   * <p>This method defines season-specific tasks to be executed during the {@link
   * SimulatedField#simulationPeriodic()} method.
   *
   * <p>For example:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>This method should be implemented in the season-specific subclass of {@link SimulatedField}
   ...
   *
   * <p>For each season-specific subclass of {@link SimulatedField}, there should be a corresponding
